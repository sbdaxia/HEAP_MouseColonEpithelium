...
```{r}
all.peaks <- readRDS("Datafiles/peaks-all-12232019.rds")
filter.peaks <- readRDS("Datafiles/peaks-filtered-12232019.rds")
```
...
prepareHOMERinput(kmer.results = hf.up.peaks.k7, 
                  ...
                  filename.tag = "joint.hf.up",
                  n.kmers = 50,
                  width = 15)

genomic.regions <- "Kmer/joint.hf.up-peaks-k7-windows.bed"
background.regions <- "Kmer/joint.hf.up-peaks-k7-windows-background.bed"
homerdir <- "Kmer/homer-denovo-output-joint.hf.up-peaks-k7"
...
mouse.mirnas <- c("let-7/miR-98", "miR-200/429", "miR-194", "miR-558", "miR-25-3p/32-5p/92-3p/363-3p/367-3p", "miR-4725", "miR-26-5p/1297/4465", "miR-3183/4723-3p/6769-3p", "miR-4737", "miR-210", "miR-3940-3p", "miR-3607-3p", "miR-2115")
...
prepareHOMERinput(kmer.results = hfk.up.peaks.k7, 
                  ...
                  filename.tag = "joint.hfk.up",
                  n.kmers = 50,
                  width = 15)

genomic.regions <- "Kmer/joint.hfk.up-peaks-k7-windows.bed"
background.regions <- "Kmer/joint.hfk.up-peaks-k7-windows-background.bed"
homerdir <- "Kmer/homer-denovo-output-joint.hfk.up-peaks-k7"
dir.create(homerdir, showWarnings = FALSE)
homer.cmd <- sprintf("findMotifsGenome.pl %s mm10 %s -bg %s -len 8 -size given -rna -noweight -minlp -5 -nlen 2 -N 200000 -bits -p 10 -cache 1000 >.homer-output 2>.err.homer-output",
                     genomic.regions, homerdir, background.regions)
## Run in terminal
...
mouse.mirnas <- c("let-7/miR-98", "miR-194", "miR-22-3p", "miR-200/429", "miR-29", "miR-26-5p/1297/4465", "miR-1185-5p/3679-5p", "miR-210-3p", "miR-4436/5000-3p")
